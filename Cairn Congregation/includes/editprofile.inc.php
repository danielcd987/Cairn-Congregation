...
*/
   if(isset($_POST['update'])){
    ...
    $userName = $_POST['Euserid']; //this calls the username column from database
    $email = $_POST['Epswd']; //this calls the password column from database
    $repswd = $_POST['Epswd2'];  //this submits password to database
    $phonenum = $_POST['Ephonenumber']; //this submits phone# to database
    $occ = $_POST['Eoccupation'];
    $age = $_POST['Esage'];
//possible errors
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)){ //sees if the sql query fails
            ...
            $resultcheck = mysqli_stmt_num_rows($stmt); //checks to see if the user exists
                ...
                //this else statement enters the values inputed into database
                    ...
                    $sql = "UPDATE users SET UserNames = ? WHERE id_users = '$userid'";
                    // email_users = ?, pwdusers = ?, phonenum = ?, occ = ?, age = ? WHERE id_users = '$userid'"; //? is a placeholder
                    $stmt = mysqli_stmt_init($conn); //refers to connection to database
                    if(!mysqli_stmt_prepare($stmt, $sql)){
                        header("Location: ../editprofile.php?error=sqlerror"); //an error with the sql queries
                        exit();
                    }
                    else{
                       //hashing password
